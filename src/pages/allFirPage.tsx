...
const AllFirPage: React.FC = () => {
  ...
  console.log(reload)
  useEffect(()=>{
    axios.get(`${backendURL}/getallfir`).then((e)=>{
      console.log(e.data.data)
      let firData = e.data.data;
      setIssueList(firData)
    }).catch((e)=>{
     console.log(e)
     alert('there is sone error feching data')
    })
    },[reload])
  ...
  return (
    <IonPage>
      <IonContent>
        <div className="box">
          <IonCardTitle>All Complains</IonCardTitle>
          {issueList.map((issue) => (
            <div key={issue.id}>
            ...
            {issue.status !== 'Discard' && canChange ? <IonButton fill="clear"  onClick={(e)=>{setDiscart(issue.id)}}>Discard</IonButton>: ''}
